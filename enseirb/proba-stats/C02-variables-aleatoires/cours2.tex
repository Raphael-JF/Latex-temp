...
\begin{remarque}{}{importante ici}
    Si $F \neq \R$ il n'y a qu'à considérer la variable aléatoire $\norme{X}_F$, bien à valeurs dans $\R_+$ et lui appliquer le résultat. Cette astuce est utile dans tous les énoncés suivants de la section.
\end{remarque}
...
\begin{proposition}{}{inégalité de Cauchy-Schwarz}
    Soit $X$ et $Y$ deux variables aléatoires réelles sommables sur $(\univ, \tribu, \proba)$ telles que $XY$ est sommable. Alors~:
    $$\vert \mb{E}(XY) \vert \leq \sqrt{\mb{E}(X^2)}\sqrt{\mb{E}(Y^2)}$$
    
\end{proposition}

\begin{remarque}{}{cas particulier}
    Pour $X$ réelle positive quelconque et $Y = \overline{1}$~:
...
\begin{demonstration}
    ...
    $$0 \leq \mb{E}(Z_t^2) = \mb{E}(X^2) + 2t\mb{E}(XY) + t^2\mb{E}(Y^2)$$
...
\begin{remarque}{}{condition suffisante pour que $XY$ soit sommable}
    Si $X$ et $Y$ sont deux variables aléatoires réelles sommables telles que $X^2$ et $Y^2$ sont sommables, alors $XY$ est sommable par inégalité de Young~:
...
\begin{definition}{}{covariance de deux variables aléatoires}
    ...
    Le résultat est bien analogue à la formule de König-Huygens.
\end{definition}
...
\begin{definition}{}{fonction génératrice pour une VARD}
    ...
    \fonction{G_X}{\R}{\R}{t}{\mb{E}\Big(X^t\Big)}
