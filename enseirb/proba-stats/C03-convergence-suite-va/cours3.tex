...
\begin{demonstration}
    \begin{itemize}
        ...
        \begin{align*}
            ...
            &\leq \esperance{\frac{\vert X_n - X\vert}{1 + \vert X_n - X\vert} \indicatrice_{\vert X_n - X \vert \geq \varepsilon}} + \frac{\varepsilon}{1+\varepsilon}\\
            &\leq \esperance{\indicatrice_{\vert X_n - X \vert \geq \varepsilon}} + \frac{\varepsilon}{1+\varepsilon}\\
            &= \probaset{\vert X_n - X \vert \geq \varepsilon} + \frac{\varepsilon}{1+\varepsilon}\\
            &\leq \varepsilon + \frac{\varepsilon}{1+\varepsilon} \qquad(\text{pour } n \text{ suffisamment grand})\\
            &\leq 2\varepsilon
        \end{align*}
        Ceci est la définition de limite nulle quand $n$ tend vers l'infini.

        \item $(2) \implies (1)$. On suppose que~:
        $$\lim_{n \to +\infty} \esperance{\frac{\vert X_n - X\vert}{1 + \vert X_n - X\vert}} = 0$$
        Soit $\varepsilon > 0$. On a~:
        $$\frac{\vert X_n - X \vert}{1 + \vert X_n - X \vert} \geq \frac{\varepsilon}{1+\varepsilon}\indicatrice_{\vert X_n - X \vert \geq \varepsilon}$$
        Et par croissance de l'espérance~:
        \begin{align*}
            \esperance{\frac{\vert X_n - X\vert}{1 + \vert X_n - X\vert}} &\geq \esperance{\frac{\varepsilon}{1+\varepsilon}\indicatrice_{\vert X_n - X \vert \geq \varepsilon}}\\
            &= \frac{\varepsilon}{1+\varepsilon}\probaset{\vert X_n - X \vert \geq \varepsilon}
        \end{align*}
        Donc, en passant à la limite quand $n$ tend vers l'infini, on obtient~:
        $$\lim_{n \to +\infty} \probaset{\vert X_n - X \vert \geq \varepsilon} = 0$$
    \end{itemize}
...
\begin{demonstration}
    Supposons que $(X_n)_{n \in \N}$ converge presque sûrement vers $X$. Soit $\varepsilon > 0$. On a~:
    $$\proba\Big(\underbrace{\{\omega \in \Omega, \lim_{n \to +\infty} X_n(\omega) = X(\omega)\}}_{:=A}\Big) = 1$$
    Pour $\omega \in A$, on a par continuité et annulation de $f:x\mapsto \frac{x}{1+x}$ en $0$~:
    $$\frac{\vert X_n(\omega)-X(\omega)\vert}{1 + \vert X_n(\omega) - X(\omega)\vert} \flechelim{n \to + \infty}{} 0$$
    Cela signifie que~:
        $$A = \{\omega \in \Omega, \lim_{n \to +\infty} \vert X_n(\omega) -  X(\omega)\vert = 0\}  \subset \{\omega \in \Omega, \lim_{n \to +\infty} f(\vert X_n(\omega) -  X(\omega)\vert) = 0\}$$
    En passant aux probabilités, on obtient~:
    $$1 = \proba(A) \leq \proba\Big(\{\omega \in \Omega, \lim_{n \to +\infty} f(\vert X_n(\omega) -  X(\omega)\vert) = 0\}\Big) = 1$$
    On a alors que~:
    $$\frac{\vert X_n-X\vert}{1 + \vert X_n - X\vert} \xrightarrow[\;n \to +\infty\;]{\mr{p.s.}} 0$$
    D'après le théorème de convergence dominée pour les variables aléatoires (on peut choisir $Y=1$), on a~:
    $$\lim_{n \to +\infty} \esperance{\frac{\vert X_n - X\vert}{1 + \vert X_n - X\vert}} = 0$$
    Ceci équivaut à la convergence en probabilité de $(X_n)_{n \in \N}$ vers $X$ d'après la proposition précédente.
\end{demonstration}
...
$$\frac{\sqrt{n}}{\sigma}\Big(\overline{X}_n - \esperance{X_1}\Big) \xrightarrow[\;n \to +\infty\;]{\mc{L}} Y$$
