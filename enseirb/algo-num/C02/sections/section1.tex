...
\begin{remarque}{}{conditionnement d'une matrice symétrique définie positive}
    Si $A \in S_n^+(\R)$, alors $\mr{cond}(A) = \frac{\lambda_{max}}{\lambda_{min}}$ où $\lambda_{max}$ et $\lambda_{min}$ sont les valeurs propres de $A$ de plus grande et plus petite valeur absolue respectivement. On caractérise alors conditionnement comme la capacité d'une matrice à convertir les boules en ellipsoïdes~:
    \begin{itemize}
        \item Plus le conditionnement est élevé, plus les boules sont transformées en ellipsoïdes allongés, et plus l'erreur est amplifiée.
        \item Plus le conditionnement est faible, plus les boules sont transformées en ellipsoïdes proches de cercles, et moins l'erreur est amplifiée.
    \end{itemize}

    Dans le cas général, le conditionnement d'une matrice $A$ est égal au rapport entre les plus grandes et les plus petites valeurs singulières de $A$ (généralisation des valeurs propres pour les matrices non carrées).
\end{remarque}
