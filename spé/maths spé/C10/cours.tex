...
\begin{theoreme}{10.73}{dérivation $k$ fois d'une limite uniforme de suite de fonctions}
    ...
    \begin{enumeratebf}
        ...
        \item pour tout $i \in \intint{0}{k-1}$, la suite $(f_n^{(i)})_{n \in \N}$ converge simplement vers une fonction $h_i \in \mc{CM}(I,E)$
        \item $(f_n^{(k)})_{n \in \N}$ converge uniformément vers une fonction $g$ sur $I$.
    \end{enumeratebf}
    la fonction $h_0$ est de classe $\mc{C}^k(I,E)$ et $h_0^{(k)} = g$.
\end{theoreme}
