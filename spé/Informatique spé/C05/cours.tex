...
\begin{implementation}{algorithme d'apprentissage non supervisé des $k$-moyennes}
    \begin{lstLNat}
    k_mean($k$, $Z$):
        // précondition : $k \leq Z$
        pour tout $i \in \intint{1}{k}$:
            $c_i$ = élément aléatoire de $Z$, différent des précédents centres
        initialiser $(c_i)_{i \in \intint{1}{k}}$ les $k$ centres des clusters comme 
                etant des éléments aléatoires de $Z$

        tant que les centres sont modifiés:
            décomposer $ Z = \bigsqcup_{i=1}^kP_i$ où $P_i$ est 
                    l'ensemble des plus proches de $c_i$ que des autres centres
            pour tout $i \in \intint{1}{k}$:
                $c_i$ = $ \frac{1}{\abs{P_i}} \sum_{z \in P_i} z$
        renvoyer $(P_i)_{i \in \intint{1}{k}}$
